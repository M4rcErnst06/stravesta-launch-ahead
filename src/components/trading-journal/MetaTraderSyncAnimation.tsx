          ...
          <div className="h-full space-y-2 overflow-hidden flex flex-col justify-start">
            {journalEntries.slice(0, 3).map((entry, index) => (
              <div 
                key={entry.id}
                className={`
                  bg-stravesta-dark/50 border border-stravesta-darkGray rounded-lg p-2 transition-all duration-700
                  ${entry.status === 'syncing' ? 'border-stravesta-teal shadow-lg shadow-stravesta-teal/20' : ''}
                  opacity-0 translate-x-8 animate-[fade-in_0.5s_ease-out_forwards] h-[50px] flex flex-col justify-center
                `}
              ...
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-1 flex-1 min-w-0">
                    ...
                    <Badge className="bg-blue-500/20 text-blue-400 border-blue-500/30 text-xs px-1 py-0 h-4 ml-1">
                      {entry.category}
                    </Badge>
                    <span className="text-stravesta-lightGray text-xs ml-1">{entry.session}</span>
                  </div>
                  <div className="flex items-center gap-1 ml-2">
                    <span className="text-stravesta-lightGray text-xs">Size: {entry.size}</span>
                    <span className={`font-bold text-xs ${entry.profit >= 0 ? 'text-green-500' : 'text-red-500'}`}>
                      ${entry.profit}
                    </span>
